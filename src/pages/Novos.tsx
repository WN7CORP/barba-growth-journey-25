...
const Novos = () => {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 pb-20">
        <Header onSearch={() => {}} onPriceFilter={() => {}} />
        <div className="container mx-auto px-4 py-8">
          <div className="animate-pulse space-y-6">
            <div className="h-32 bg-white/20 rounded-2xl animate-shimmer"></div>
            <div className="space-y-3">
              {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-24 bg-white/20 rounded-2xl animate-shimmer"></div>)}
            </div>
          </div>
        </div>
      </div>;
  ...
  return <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 pb-20">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-6 animate-fade-in">
          <Button variant="ghost" size="sm" onClick={() => navigate('/')} className="text-white hover:bg-white/20 rounded-xl transition-all duration-300 hover:scale-105">
        ...
        {products.length === 0 ? <div className="text-center py-16 animate-fade-in">
      ...
      <div className="space-y-4">
            {products.map((product, index) => <div key={product.id} className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 hover:bg-white/20 transition-all duration-300 animate-fade-in" style={{
          animationDelay: `${index * 0.1}s`
        }}>
                <div className="flex gap-4">
                  <div className="w-20 h-28 flex-shrink-0 rounded-lg overflow-hidden">
                    <OptimizedImage src={product.imagem1} alt={product.produto} className="w-full h-full object-cover" />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2">
                      <Button size="sm" onClick={() => handleProductClick(product)} className="bg-white/20 text-white hover:bg-white/30 text-xs px-3 py-1 h-auto">
                        Ver mais
                      </Button>
                      
                      {product.video}
                      
                      <Button size="sm" asChild className="bg-green-500 hover:bg-green-600 text-white text-xs px-3 py-1 h-auto">
      ...
      {selectedProduct && <ProductDetailModal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} product={selectedProduct} />}
    </div>;
};
